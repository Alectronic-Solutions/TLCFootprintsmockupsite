...
// Sole spans y 18.0 (ball, where the toes attach) to 46.8 (heel). Split into
// even thirds - ball / arch / heel, top to bottom.
 ...
 * SVG. The three sole bands share the sole's full x-extent (roughly 8.5–32.5)
 * rather than a hand-fitted narrower box - the sole is narrowest near the
 * toes and widest at the ball, so a per-band x-fit would clip the ball
 * region's own curve. A little extra air on every side costs nothing at
 * tile scale and guarantees nothing is cropped.
 */
export const REGION_VIEWBOX: Record<FootRegion, string> = {
  ball: `7.5 ${BALL_BAND.y - 0.8} 25 ${BALL_BAND.height + 1.6}`,
